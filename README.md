...
- `Группа` — выпадающий список (Материалы, Заказчики, Сотрудники, Счета, Субподрядчики, Контрагенты).
...
3. Выберите группу из списка или используйте быстрый ввод одной строкой.
...
После выбора `Пополнение` или `Расход` можно сразу отправить строку:
...
```text
1 2 продажа двери 100000 373
```

Номера групп и назначений — в `/справка`.
